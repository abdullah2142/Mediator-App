...
<head>
    ...
    <title>Create Session - MediAItor</title>
    ...
    <main class="max-w-md mx-auto px-4 py-16">
        ...
        <div class="bg-white rounded-2xl shadow-xl p-8">
            ...
            <form method="POST" action="{{ route('session.store') }}">
                ...
                <div class="mb-6">
                    <label for="conflict_type" class="block text-sm font-medium text-slate-700 mb-2">Conflict Type</label>
                    <select id="conflict_type" name="conflict_type" required
                            class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500">
                        <option value="">Select a type...</option>
                        <option value="relationship" {{ old('conflict_type') == 'relationship' ? 'selected' : '' }}>Relationship</option>
                        <option value="family" {{ old('conflict_type') == 'family' ? 'selected' : '' }}>Family</option>
                        <option value="roommate" {{ old('conflict_type') == 'roommate' ? 'selected' : '' }}>Roommate</option>
                        <option value="workplace" {{ old('conflict_type') == 'workplace' ? 'selected' : '' }}>Workplace</option>
                        <option value="friendship" {{ old('conflict_type') == 'friendship' ? 'selected' : '' }}>Friendship</option>
                        <option value="other" {{ old('conflict_type') == 'other' ? 'selected' : '' }}>Other</option>
                    </select>
                </div>
            ...
            <div class="mt-6 text-center">
                <p class="text-slate-500 text-sm">
                    Have a code? <a href="{{ route('session.join') }}" class="text-teal-600 hover:underline">Join a session</a>
                </p>
            </div>
        </div>
    </main>
